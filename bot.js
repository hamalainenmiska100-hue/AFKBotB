...
function actualConnect(uid, interaction) {
  const u = getUser(uid);
  interaction.editReply("⏳ Connecting to server... 🔌").catch(() => {});
...
client.on(Events.InteractionCreate, async (i) => {
    ...
    if (i.commandName === "admin" && i.channelId === ADMIN_CHANNEL_ID) {
      ...
      if (sub === "info") {
        ...
        const emb = new EmbedBuilder().setTitle("🖥 Admin Panel").setColor(0x00FF00)
          .addFields({ name: "🧠 RAM Usage", value: `${mem.toFixed(1)}MB`, inline: true }, { name: "🎮 Active Bots", value: `${sessions.size}`, inline: true })
  ...
  if (i.isButton()) {
    ...
    if (i.customId === "start") {
      ...
      if (!u.server?.ip) return i.editReply("❌ Configure server IP in **Settings** first. ⚙");
      ...
      if (now - u.lastAdTime > AD_COOLDOWN_MS) {
        ...
        const adRow = new ActionRowBuilder().addComponents(
          new ButtonBuilder().setLabel("🔗 Open Reward Link").setStyle(ButtonStyle.Link).setURL(result.url),
    ...
    if (i.customId === "stop") { stopSession(uid); return i.reply({ ephemeral: true, content: "⏹ Bot stopped." }); }
    if (i.customId === "unlink") { const u = getUser(uid); u.linked = false; save(); return i.reply({ ephemeral: true, content: "🗑 Account unlinked." }); }
