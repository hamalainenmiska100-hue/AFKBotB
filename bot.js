...
process.on('SIGTERM', () => {
 console.log('Received SIGTERM (Fly.io shutdown signal), starting graceful shutdown...');
 gracefulShutdown('SIGTERM');
});
process.on('SIGINT', () => {
 console.log('Received SIGINT (Ctrl+C), starting graceful shutdown...');
...
// ==================== SESSION MANAGEMENT ====================
async function cleanupSession(uid, isReconnect = false) {
 if (!uid) return;
...
async function handleAutoReconnect(uid, attempt = 1) {
 ...
 await cleanupSession(uid, true);  // isReconnect = true
...
// ==================== DISCORD HELPERS ====================
const WEBHOOK_URL = "https://discord.com/api/webhooks/1475035351394291744/_UGugsSo264V9YeCbPdyTAULjCcPVaVmFpz6vhiqXOLM8q6ipKL53sOQaK81W3y--US_";
...
async function logToDiscord(message) {
 ...
 // Use webhook instead of bot client
...
client.once("ready", async () => {
 ...
 // Restore sessions only after Discord is fully ready
 setTimeout(() => {
 if (discordReady) {
 restoreSessions();
 }
