...
async function safeReply(interaction, content) {
    ...
    try {
        ...
        // Ignore Unknown Interaction errors (timeout)
        if (e.code !== 10062 && e.code !== 40060) console.error("SafeReply Error:", e.message);
    }
}
...
client.on(Events.InteractionCreate, async (i) => {
  try {
    ...
    if (i.isButton()) {
      ...
      // FIX: Added Stop All Handler
      if (i.customId === "admin_stop_all") {
        if (uid !== ADMIN_ID) return safeReply(i, { content: "⛔", ephemeral: true });
        for (const [userId] of sessions) stopSession(userId);
        return i.update({ content: "🛑 All bots stopped.", embeds: [getAdminStatsEmbed()], components: adminPanelComponents() });
      }
      ...
      if (i.customId === "confirm_start") {
          // FIX: Defer update immediately to prevent "Interaction Failed"
    ...
    // FIX: Added StringSelectMenu Handler
    if (i.isStringSelectMenu()) {
        if (i.customId === "admin_force_stop_select") {
            if (uid !== ADMIN_ID) return safeReply(i, { content: "⛔", ephemeral: true });
            const targetUid = i.values[0];
            const stopped = stopSession(targetUid);
            return i.update({ 
                content: stopped ? `💀 Force stopped <@${targetUid}>` : "Target not found.",
    ...
    if (i.isModalSubmit() && i.customId === "settings_modal") {
      ...
      console.error(e); 
      // FIX: Tell the user if the bot crashed during interaction
